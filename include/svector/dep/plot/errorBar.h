	...
	namespace plot
	{
		...
		public:
			HorizontalErrorBars(std::span<const double> xs, std::span<const double> ys, std::span<const double> plusErrors, std::span<const double> minusErrors, std::shared_ptr<Axis> xAxis, std::shared_ptr<Axis> yAxis, Length stopLength, const LineStyle style = sci::plot::LineStyle(), bool useForAutoscale = true, std::shared_ptr<splotTransformer> transformer = nullptr)
				...
				Data<std::vector<double>, std::vector<double>, std::vector<double>, std::vector<double>>({ xAxis, yAxis, xAxis, xAxis }, transformer, xs, ys, (xs | sci::views::grid<1>) + (plusErrors | sci::views::grid<1>), (xs | sci::views::grid<1>) - (minusErrors | sci::views::grid<1>))
			{
				m_style = style;
			...
			void plotData(Renderer& renderer, grPerMillimetre scale) const override
			{
				...
				const std::vector<double>& ys = getData<1>();
				const std::vector<double>& minXs = getData<2>();
				const std::vector<double>& maxXs = getData<3>();
			...
			virtual void autoscaleAxes() override
			{
				if (!m_useForAutoscale)
					return;
				Data<std::vector<double>, std::vector<double>, std::vector<double>, std::vector<double>>::autoscaleAxes();
			}
		private:
			LineStyle m_style;
			Length m_stopLength;
		...
		public:
			VerticalErrorBars(std::span<const double> xs, std::span<const double> ys, std::span<const double> plusErrors, std::span<const double> minusErrors, std::shared_ptr<Axis> xAxis, std::shared_ptr<Axis> yAxis, Length stopLength, const LineStyle style = sci::plot::LineStyle(), bool useForAutoscale = true, std::shared_ptr<splotTransformer> transformer = nullptr)
				: PlotableItem(xAxis, yAxis, transformer),
				Data<std::vector<double>, std::vector<double>, std::vector<double>, std::vector<double>>({ xAxis, yAxis, yAxis, yAxis }, transformer, xs, ys, (ys | sci::views::grid<1>) + (plusErrors | sci::views::grid<1>), (ys | sci::views::grid<1>) - (minusErrors | sci::views::grid<1>))
			{
				m_style = style;
				m_stopLength = stopLength;
				m_useForAutoscale = useForAutoscale;
			...
			void plotData(Renderer& renderer, grPerMillimetre scale) const override
			{
				...
				const std::vector<double>& xs = getData<0>();
				const std::vector<double>& ys = getData<1>();
				const std::vector<double>& minYs = getData<2>();
				const std::vector<double>& maxYs = getData<3>();
