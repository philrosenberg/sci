...
template<>
inline double NumberFromTextValidator<double>::textToNumber(const wxString& text) const
{
	double result;
...
template<>
inline float NumberFromTextValidator<float>::textToNumber(const wxString& text) const
{
	double result;
	if (!text.ToDouble(&result))
