...
namespace sci
{
	...
	auto max(const T& v)
	{
		...
		if (std::ranges::size(v) == 0)
			return std::numeric_limits<retType>::quiet_NaN();
		retType result = std::numeric_limits<retType>::lowest();
		if constexpr (std::numeric_limits<retType>::has_infinity && std::numeric_limits<retType>::is_signed)
			result = -std::numeric_limits<retType>::infinity();
		for (auto vi : v)
			if (vi > result)
				result = vi;
		return result;
	...
	*/
	...
	auto medianInPlace(RANGE& v)
	{
		...
		if (v.size() % 2 == 1)
			return (value_type)findKthBiggestValue(v, v.size() / 2);
